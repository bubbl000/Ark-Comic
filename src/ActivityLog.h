...
public:
    ...
    // 记录一条活动日志（线程安全）
    // category: 分类标签（如 L"加载" L"解码" L"翻页" L"缓存" L"资源库" L"UI"）
    // msg: 详细消息
    ...
    // 注册快照采集器：内部 1s 定时器调用（_perfEnabled 时），由主窗口注册
...
// ─── 跨窗口快照状态 ───
// MainWindow/ReaderWindow 各自更新字段，快照采集器每秒读取；解码线程会改 cache 字段，故加锁
namespace PerfState {
    struct Snapshot {
        std::wstring libraryPath;   // 当前资源库路径
        std::wstring comicTitle;    // 当前打开漫画标题
        int currentPage = -1;       // 当前页码（0 基）
        int totalPages = 0;         // 总页数
        size_t cacheEntries = 0;    // 解码缓存条目数
        double cacheMemMB = 0;      // 解码缓存内存估算
    };
    std::mutex& Mutex();            // 保护 Current()
    Snapshot& Current();            // 全局快照状态
}
